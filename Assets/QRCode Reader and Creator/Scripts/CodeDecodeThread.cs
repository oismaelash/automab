...
//CodeDecodeThread handles threading
public class CodeDecodeThread : MonoBehaviour
{
	private static CodeDecodeThread _current;
		
	public static CodeDecodeThread Current
	{
		get
		{
			if (_current == null && Application.isPlaying)
			{
				...
				var g = GameObject.Find("CodeDecodeThread");
				if(g==null)
				{
					g = new GameObject("CodeDecodeThread");
				}
				
				_current = g.GetComponent<CodeDecodeThread>() ?? g.AddComponent<CodeDecodeThread>();
			}
